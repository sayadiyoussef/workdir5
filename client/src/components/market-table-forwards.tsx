...
// Forwards – extend per-grade when available. Empty arrays are supported for other grades.
            ...
            <tbody className="bg-trading-slate">
              {isLoading ? (
                ...
                latestData.map((item:any) => {
                  ...
                  const fw = forwardPrices[item.gradeName] || [];
                  return (
                    <>
                      <tr
                        key={item.id}
                          ...
                          <td className="px-6 py-3 bg-gray-900/60" colSpan={6}>
                            {fw.length === 0 ? (
                              ...
                              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3">
                                {fw.map((p, idx) => (
                                  <div key={idx} className="border border-gray-700 rounded-lg p-3 bg-gray-800/60 hover:bg-gray-800 cursor-pointer" onClick={()=> setLocation(`/fixings?price=${p.price}&grade=${encodeURIComponent(item.gradeName)}&period=${encodeURIComponent(p.label)}`)}>
